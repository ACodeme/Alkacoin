...
class Blockchain:
	def __init__(self, public_key, node_id):
		genesis_block = Block(0, '', [], 100000000000000000000000000, 0)
					...
					try:
						...
						responce = requests.post(
							...
							json={
								...
								'amount': amount,
								'signature': signature})
						if responce.status_code == 400 or responce.status_code == 500:
							print('No transaction resolve it !!!!')
							return False
	...
	def mine_block(self):
		...
		self.__open_transactions = []
		self.save_data()
		for nodes in self.__peer_nodes:
			url = 'http://{}/broadcast-block'.format(node)
			converted_block = block.__dict.copy()
			converted_block['transactions'] = [tx.__dict__ for tx in converted_block['transactions']]
			try:
				responce = requests.post(url, json={'block': converted_block})
				if responce.status_code == 400 or responce.status_code == 500:
		...
		return block
		print('-'*30)
		print(f"{self.public_key} has mined a new block.")
	...
	def add_block(self, block):
		...
		for itx in blovk['transactions']:
			for opentx in stored_transactions:
				if opentx.sender == itx['sender'] and opentx.recipient == itx['recipient'] and opentx.amont == itx['amount'] and opentx.signature == itx['signature']:
					try:
